...
class NotifRoutes(Blueprint):
    ...
    def register_routes(self):
        ...
        self.route('/apinotif/notif/<int:list_id>/<int:task_id>', methods=['GET'])(self.get_notif_by_id)
        
        self.route('/apinotif/notif/<int:list_id>/<int:task_id>', methods=['PUT'])(self.update_due_notif)
    ...
    def get_notif_by_id(self, list_id, task_id):
        try:
            self.notif = self.notif_service.get_notif_by_id(list_id, task_id)
            if self.notif:
                return jsonify(self.notif), 200
            else: 
    ...
    def update_due_notif(self, list_id, task_id):
        try:
            ...
            self.notif = self.notif_service.update_due_notif(list_id, task_id, self.due_notif)
            if self.notif:
                return jsonify(self.notif), 200
            else: 
                return jsonify({'error': 'Notification not found'}), 404
